...
import { Search, Menu, X, ArrowRight, RefreshCw } from 'lucide-react';
  ...
  return (
    <>
        ...
        <div className="flex flex-col gap-2">
          ...
          >
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <div className="w-9 h-9 bg-gradient-to-tr from-[#1d1d1f] to-stone-800 text-white rounded-full flex items-center justify-center font-serif text-lg font-bold shadow-sm transition-transform duration-300 group-hover/logo:scale-105">
                  CK
                </div>
                <div className="flex flex-col text-left">
                  <span className="text-sm font-semibold tracking-tight text-[#1d1d1f] leading-none">
                    Click Karo <span className="font-serif italic font-normal text-pink-600">Date Karo</span>
                  </span>
                  <span className="text-[10px] text-[#86868b] font-medium tracking-wide">
                    Social Support India
                  </span>
                </div>
              </button>
              ...
              <div className="flex items-center gap-1">
                <a 
                  href="#services" 
                  onClick={() => onSwitchMode('landing')}
                  className="text-xs font-medium text-[#1d1d1f]/80 hover:text-[#0071e3] transition-colors px-3 py-1.5 rounded-full hover:bg-black/[0.04] apple-focus"
                >
                  Services
                </a>
                <a 
            ...
            <div className="flex items-center gap-2">
              ...
              {/* DUAL MODE INSTANT SWITCHER PILL */}
              {currentRole ? (
                <div className="flex items-center bg-white/90 p-1 rounded-full border border-pink-200 shadow-sm">
                  {userName && (
                    <span className="text-[11px] font-bold text-[#1d1d1f] px-2 py-0.5 border-r border-pink-200">
                      {userName.split(' ')[0]}
                    </span>
                  )}
                  <button
                ...
                <button 
                  ...
                  className="text-xs font-semibold text-[#1d1d1f] hover:text-[#0071e3] px-3.5 py-2 rounded-full border border-pink-200 hover:border-pink-300 hover:bg-white transition-all apple-focus"
                >
                  Sign In / Join
                </button>
              )}
              ...
              <button 
                ...
                className="bg-[#0071e3] hover:bg-[#0077ed] text-white text-xs font-semibold px-4 py-2 rounded-full shadow-apple-sm hover:shadow-apple-md active:scale-95 transition-all flex items-center gap-1.5 apple-focus"
              >
      ...
      <nav 
        ...
        className="lg:hidden fixed top-3 left-4 right-4 z-50 h-14 apple-glass rounded-full flex items-center justify-between px-4 shadow-apple-sm"
      >
        <button onClick={() => onSwitchMode('landing')} className="flex items-center gap-2 apple-focus rounded-full p-1 text-left">
          <div className="w-8 h-8 bg-[#1d1d1f] text-white rounded-full flex items-center justify-center font-serif text-sm font-bold">
            CK
          </div>
          <span className="text-xs font-bold text-[#1d1d1f] tracking-tight">Click Karo</span>
        </button>

        <div className="flex items-center gap-2">
          {currentRole ? (
            <div className="flex bg-white/90 p-0.5 rounded-full border border-pink-200">
              <button 
                onClick={() => onSwitchMode(currentView === 'seeker' ? 'companion' : 'seeker')}
                className="bg-[#1d1d1f] text-white text-[10px] font-bold px-2.5 py-1 rounded-full shadow-sm flex items-center gap-1"
              >
                <RefreshCw className="w-2.5 h-2.5" />
                <span>{currentView === 'seeker' ? 'Go Companion' : 'Go Seeker'}</span>
              </button>
            </div>
          ) : (
            <button 
              onClick={() => onOpenAuth('signin')}
              className="bg-pink-100 text-pink-900 text-[11px] font-bold px-3 py-1 rounded-full"
            >
              Sign In
            </button>
          )}
          ...
          <button 
            onClick={onOpenBooking}
            className="bg-[#0071e3] text-white text-xs font-medium px-3 py-1.5 rounded-full shadow-sm active:scale-95 apple-focus"
          >
            Find Partner
          </button>
          <button 
            ...
            className="w-8 h-8 rounded-full bg-black/[0.05] text-[#1d1d1f] flex items-center justify-center apple-focus"
          >
      ...
      >
        ...
        <div className="flex flex-col gap-4 overflow-y-auto no-scrollbar">
          ...
          >
            ...
            <span className="text-xs bg-[#f5f5f7] text-[#86868b] px-2 py-0.5 rounded-full font-normal">16 Options</span>
          </a>
          <a 
            href="#earnings" 
            onClick={() => { setMobileMenuOpen(false); onSwitchMode('landing'); }}
