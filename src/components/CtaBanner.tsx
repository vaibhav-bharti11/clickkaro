...
export const CtaBanner: React.FC<CtaBannerProps> = ({ onOpenBooking, onOpenPartnerJoin }) => {
  ...
  const ctaCms = content.ctaBanner;

  return (
    <section className="py-28 px-4 sm:px-6 relative overflow-hidden text-center bg-[#121214] text-white font-sans">
      {/* Ambient Radial Glow */}
      ...
      <div className="max-w-4xl mx-auto flex flex-col items-center relative z-10">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-white/20 bg-white/10 backdrop-blur-md mb-6">
          ...
          <span className="text-xs font-semibold text-white tracking-wide uppercase">
            {ctaCms.badge || "India's #1 Social & Lifestyle Support Network"}
          </span>
        </div>

        {/* Master Heading */}
        <h2 className="text-3xl sm:text-5xl md:text-6xl font-semibold tracking-tight text-white mb-6 headline-balance leading-[1.08]">
          {ctaCms.title || 'Find a companion for every occasion, in any city across India.'}
        </h2>

        <p className="text-white/70 text-base sm:text-lg mb-10 leading-relaxed max-w-2xl body-pretty">
          {ctaCms.subtitle || 'Stop missing out on cinema screenings, casual hangouts, dinner outings, or travel getaways alone. Connect with background-checked, verified companions across India.'}
        </p>
        ...
        <div className="flex flex-col sm:flex-row gap-3.5 w-full sm:w-auto mb-10">
          <button
            ...
            className="bg-[#FF2D55] hover:bg-[#E11D48] text-white font-semibold text-sm px-9 py-4 rounded-full transition-all duration-300 shadow-apple-md hover:shadow-apple-lg active:scale-95 flex items-center justify-center gap-2 apple-focus cursor-pointer"
          >
            <span>{ctaCms.primaryButtonText || 'Find a Companion Now'}</span>
            <ArrowRight className="w-4 h-4" />
          </button>

          <button
            onClick={onOpenPartnerJoin}
            className="bg-white/10 hover:bg-white/20 border border-white/20 text-white font-semibold text-sm px-8 py-4 rounded-full transition-all active:scale-95 flex items-center justify-center gap-2 apple-focus cursor-pointer"
          >
            <UserCheck className="w-4 h-4 text-[#FF2D55]" />
            <span>{ctaCms.secondaryButtonText || 'Join as Partner (Earn ₹7-10k/day)'}</span>
          </button>
        </div>
